...
或写成MOV [BX].2623H,AX  
```
已知(DS)=1500H，(BX)=6854H，则有效地址EA=(BX)+2623H=8E77H 物理地址=(DS)x10H+EA=15000H+8E77H=1DE77H  
...
MOV AX,[BX+DI]
```
执行前:已知(DS)=2100H,(BX)=0158H,(DI)=10ASH,(221FD)=34H,(221FE)=95H,(AX)=0FFFFH。则 有效地址EA=(BX)+(DI)=0158H+10A5H=11FDH 物理地址=(DS)x10H+EA=21000H+11FDH=22IFDH
...
MOV AX,[MASK+BX+SI]  默认DS寄存器作段地址  
MOV AX,[BX+SI] MASK  i默认DS寄存器作段地址
```
以上3种表示形式实现的功能是一样的。 其有效地址EA=MASK+(BX)+(SD);物理地址=(DS)X10H+EA。
